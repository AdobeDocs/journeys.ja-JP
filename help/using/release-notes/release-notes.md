...
最新の[ドキュメントの更新](../release-notes/documentation-updates.md)を参照することもできます。
...
<tr>
<td>
<p>新しいアクションアクティビティを使用すると、個々の人を別の遍歴に移動させることができます。 <strong>Jump</strong>アクティビティを使用すると、次のことができます。
</p>
<ul>
...
<tr>
<td>
<p>eventIDを使用せずに、イベントをより簡単に設定できる新しいメソッドが追加されました。ルールベースのイベントは、条件に従ってイベントをトリガーする必要があるかを評価します。 「システム生成」と呼ばれる既存のメソッドを引き続き使用できます。 この機能は、アルファプログラムを使用して限られたお客様の間でテスト済みで、すべてのお客様がベータ版で利用できるようになりました。
</p>
...
**セグメント資格**&#x200B;アクティビティは、Campaign Standardメッセージアクティビティを含む遍歴では使用できなくなりました。 この制限により、Adobe Campaign Standardインスタンスの整合性が保護されます。 実際、セグメントの資格の使用は、メッセージ送信を毎日ピークに達し、Campaign Standardのトランザクションメッセージングに大きな負荷がかかる可能性があります。 [詳細を表示](../about/limitations.md#segment-qualification)
