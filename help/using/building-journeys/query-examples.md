...
クエリは、セグメントエクスポートジョブがキューに入れられた時刻と最終的に終了した時刻との間の時間差（分単位）を返します。

### 重複しているので、ジャーニーで破棄されたプロファイルの数
...
クエリは、重複しているので、ジャーニーによって破棄されたすべてのプロファイル ID を返します。
...
クエリは、無効な名前空間が存在するか、その名前空間の ID がないので、ジャーニーによって破棄されたすべてのプロファイル ID を返します。

### ID マップがないので、ジャーニーによって破棄されたプロファイルの数
...
ID マップが見つからなかったので、クエリは、ジャーニーによって破棄されたすべてのプロファイル ID を返します。

### ジャーニーがテストノードにあり、プロファイルがテストプロファイルでなかったので、ジャーニーによって破棄されたプロファイルの数
...
書き出しジョブはテストモードで実行されたが、プロファイルの testProfile 属性が true に設定されていなかったので、クエリは、ジャーニーによって破棄されたすべてのプロファイル ID を返します。

### 内部エラーが原因でジャーニーによって破棄されたプロファイルの数
...
クエリは、一部の内部エラーが原因でジャーニーによって破棄されたすべてのプロファイル ID を返します。
...
指定されたジャーニーバージョンに関連するすべてのサービスイベントを返します。 我々は、次の操作の連鎖に従うことができる。

* トピック作成
* 書き出しジョブの作成
* 書き出しジョブの終了（書き出されたプロファイルの指標を含む）
* 作業者の処理の終了

また、次のような問題も検出できます。

* トピックまたは書き出しジョブの作成でのエラー（セグメント書き出し API 呼び出しのタイムアウトを含む）
* 停止可能な書き出しジョブ（特定のジャーニーバージョンの場合、書き出しジョブの終了に関するイベントはありません）
* 作業者の問題（書き出しジョブ終了イベントを受け取ったが、作業者処理終了イベントを受け取っていない場合）

重要：このクエリが返すイベントがない場合、次のいずれかの理由が原因である可能性があります。

* ジャーニーのバージョンがスケジュールに到達していません
* ジャーニーバージョンでオーケストレーターを呼び出して書き出しジョブのトリガーを設定すると想定されている場合、アップストリームフローで問題が発生しました。ジャーニーのデプロイメント、ビジネスイベント、スケジューラーに関する問題。
...
* トピックまたは書き出しジョブの作成中にエラーが発生しました
* 書き出しジョブはまだ実行中です

### 書き出されたプロファイルに関する指標（各書き出しジョブの破棄および書き出しジョブ指標を含む）の取得
...
### すべての書き出しジョブの集計指標（セグメントの書き出しジョブと破棄）の取得
...
実行可能なジョブ（繰り返しジャーニーの場合、トピックの再利用をトリガーしたビジネスイベント）に関係なく、特定のジャーニーバージョンの全体的な指標を返します。
...
このクエリは、セグメントの適合が間違っているので、ジャーニーバージョンによって破棄されたすべてのプロファイル ID を返します。

## イベントベースのクエリ {#event-based-queries}

### ビジネスイベントがジャーニーに対して受け取られたかどうかを確認します
...
### ノード/ステータスを使用して特定の期間にジャーニーから離脱したプロファイルの数
